...
uint32_t LV_ATTRIBUTE_TIMER_HANDLER lv_timer_handler(void)
{
    ...
    if(already_running) {
        ...
        //ESP_LOGI(TAG, "already_running = 1");
        return 1;
    }
    ...
    if(lv_timer_run == false) {
        ...
        //ESP_LOGI(TAG, "lv_timer_run = false");
        return 1;
    }
    ...
    if(handler_start == 0) {
        ...
        //ESP_LOGI(TAG, "handler_start = false");
    }
    ...
    if(idle_period_time >= IDLE_MEAS_PERIOD) {
        ...
        // ESP_LOGI(TAG, " idle_period_time >= IDLE_MEAS_PERIOD");
    }
