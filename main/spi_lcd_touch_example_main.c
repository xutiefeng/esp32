...
static void example_lvgl_flush_cb(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    // hs note: Flush the updated bitmap.
    // ESP_LOGI(TAG, "x1:%d, x2=%d, y2=%d, y2=%d.",offsetx1, offsetx2, offsety1, offsety2);
    // uint32_t i;
    // static uint16_t color = 0;
    // for(i = 0;i < 2500; i++)
    //     color_map[i].full = color++;
    // esp_lcd_panel_draw_bitmap(panel_handle, 0, 0, 50, 50, color_map);
}
...
static void i2c_test_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "i2c_touch_task");
    w_data = 0;
    // 设置正常操作模式
    i2c_master_write_slave(FI_DEVIDE_MODE, I2C_ADDR_FT6336, &w_data, 1);
    w_data = 22;
    // 设置触摸有效值22，越小越灵敏
    i2c_master_write_slave(FI_ID_G_THGROUP, I2C_ADDR_FT6336, &w_data, 1);
    i2c_master_read_slave(FI_ID_G_THGROUP, I2C_ADDR_FT6336, &r_data, 1);
    printf("init THGROUP w_data= %d \n", w_data);
    printf("init THGROUP r_data= %d \n", r_data);
    // 设置激活周期 不能小于12 最大14
    i2c_master_write_slave(FI_ID_G_PERIODACTIVE, I2C_ADDR_FT6336, &w_data, 1);
    i2c_master_read_slave(FI_ID_G_PERIODACTIVE, I2C_ADDR_FT6336, &r_data, 1);
    printf("init PERIODACTIVE w_data= %d \n", w_data);
    printf("init PERIODACTIVE r_data= %d \n", r_data);
    w_data = 0;
    // 中断产生方式 持续电平
    i2c_master_write_slave(FI_ID_G_MODE, I2C_ADDR_FT6336, &w_data, 1);
    i2c_master_read_slave(FI_ID_G_MODE, I2C_ADDR_FT6336, &r_data, 1);
...
static void example_lvgl_touch_cb(lv_indev_drv_t *drv, lv_indev_data_t *data)
{
    ...
    if (gTPS.pressed == 1)
    {
        data->point.x = gTPS.x[0];
        data->point.y = gTPS.y[0];
...
void app_main(void)
{
    ...
    // Attach the LCD to the SPI bus创建接口设备
    ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)LCD_HOST, &io_config, &io_handle));

    esp_lcd_panel_handle_t panel_handle = NULL;
    esp_lcd_panel_dev_config_t panel_config = {
        .reset_gpio_num = EXAMPLE_PIN_NUM_LCD_RST,
        //.rgb_endian = LCD_RGB_ENDIAN_BGR,LCD_RGB_ELEMENT_ORDER_RGB
...
// #endif // CONFIG_EXAMPLE_LCD_TOUCH_CONTROLLER_STMPE610
    ...
    // lv_demo_benchmark();        // lvgl的demo测刷新速度
    // lv_demo_music();            // lvgl的demo音乐
